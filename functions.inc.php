...
/**
 * Regroup selectors and their properties in a way so that they use as little as possible space. 
 * For example :
 * ---
 * a {
 * background:blue;
 ...
 * Array
 * (
 *   [font-family:arial,sans-serif] => Array
 *       (
 *           [a] => Array
 *               (
 *                   [0] => font-family
...
function tc_make_attributesArray($selectorsArray)
{
	$attributesArray = array();
	foreach($selectorsArray as $key => $value)
	{
		foreach($value as $attributeKey => $attributeValue)
		{
			$attributesArray[$attributeKey.':'.$attributeValue]['a'] = array($attributeKey, $attributeValue); // to remove explode later...
			$attributesArray[$attributeKey.':'.$attributeValue]['s'][] = $key;
		}
	}
